...
- **SPI SCLK = 0.5 MHz** *(amendment, post-Phase 7, per user — was 8 MHz)*: `ADC_SCLK_DIV = 32` on the 16 MHz SMCLK. This is the bottom of the ADC's half-clock range, taken for wiring margin: 2 µs per bit, and the ~190 ns CONVST/RD strobes become shorter than one CLOCK period outright, which retires risk A below on its own terms rather than by the gated-clock argument. A tick's bus traffic grows from ~20 µs to ~135 µs — still 1.4 % of the 10 ms period.
- **Timebase**: **no crystal** *(amendment, post-Phase 7, per user)*. LFXT and HFXT are held off, PJ.4/PJ.5 stay plain GPIO and the LaunchPad's Y4 is unused; ACLK is parked on the VLO. Everything derives from the internal DCO: MCLK 16 MHz, SMCLK 16 MHz.
- **Two timings only** *(amendment, post-Phase 7, per user)*: the design has exactly two rates — a **16 MHz** system clock (MCLK = SMCLK, undivided) and the **0.5 MHz** ADC bus clock (eUSCI /32). SMCLK is no longer halved, and **no low-power mode is used anywhere**: the CPU runs continuously at 16 MHz and busy-waits on the tick flag between ticks instead of sleeping.
- **Rate**: Timer_A0 up mode on **SMCLK/8 = 2 MHz**, CCR0=19999 (period 20000) → **exactly 100.000 Hz**, to DCO accuracy (~±2 %). The tick only spaces the ADC bursts; nothing measures absolute time from it. Boot no longer spends up to a second waiting for a crystal to start, and `ST_NO_LFXT` is retired.
...
3. t2/t3 strobe-width spec (≤1 t_CLK) assumes a free-running clock; with gated clock no edges occur during the strobe. Bring-up risk A; mitigation is one define (`ADC_SCLK_DIV` → 4 or 8).
...
**Work**: Timer_A0 up mode, **SMCLK/8** (`TASSEL__SMCLK|ID__8`), `TA0CCR0=19999` → 16 MHz/8/20000 = exactly 100 Hz at DCO accuracy; the CCR0 ISR just sets the tick flag and returns — *amended: no low-power mode*, the CPU never sleeps, so main() busy-waits on the flag and there is no mode for the ISR to exit. Single timebase — the crystal path and its fallback are both gone. Toggle the heartbeat LED every 50 ticks.
